...
def ITML_experiment(K_list):
    ...
    itml_acc_list = []
    itml = ITML(gamma=1, max_iter=1000, convergence_threshold=1e-3, verbose=True, random_state=1234)
    itml.fit(pairs=train_pairs, y=np.squeeze(y_pairs))
    itml_metric = itml.get_metric()
    for K in K_list:
        knn_itml = KNeighborsClassifier(n_neighbors=K, metric=itml_metric)
        knn_itml.fit(PCA_reduced_x_train, PCA_reduced_y_train)
        Y_pred_mmc = knn_itml.predict(PCA_reduced_x_test)
        itml_acc_list.append(metrics.accuracy_score(PCA_reduced_y_test, Y_pred_mmc))
        print("n_neighbours||K: ", K, "Accuracy:", itml_acc_list[-1])
    itml_plotting(itml_acc_list)


def itml_plotting(acc_list):
    print(acc_list)
