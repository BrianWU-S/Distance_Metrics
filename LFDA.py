...
def LFDA_experiment(K_list):
    ...
    lfda_acc_list = []
    lfda = LFDA(k=7, n_components=49)
    lfda.fit(PCA_reduced_x_train, PCA_reduced_y_train)
    lfda_metric = lfda.get_metric()
    for K in K_list:
        knn_lfda = KNeighborsClassifier(n_neighbors=K, metric=lfda_metric)
        knn_lfda.fit(PCA_reduced_x_train, PCA_reduced_y_train)
        Y_pred_lfda = knn_lfda.predict(PCA_reduced_x_test)
        lfda_acc_list.append(metrics.accuracy_score(PCA_reduced_y_test, Y_pred_lfda))
        print("n_neighbours||K: ", K, "Accuracy:", lfda_acc_list[-1])
    lfda_plotting(lfda_acc_list)


def lfda_plotting(acc_list):
    print(acc_list)
