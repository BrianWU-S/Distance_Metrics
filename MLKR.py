...
def MLKR_experiment(K_list):
    ...
    mlkr_acc_list = []
    mlkr = MLKR(max_iter=1000, init='auto', n_components=50, random_state=1234)     # fine-tune the n_components
    mlkr.fit(PCA_reduced_x_train, PCA_reduced_y_train)
    mlkr_metric = mlkr.get_metric()
    for K in K_list:
        knn_mlkr = KNeighborsClassifier(n_neighbors=K, metric=mlkr_metric)
        knn_mlkr.fit(PCA_reduced_x_train, PCA_reduced_y_train)
        Y_pred_mlkr = knn_mlkr.predict(PCA_reduced_x_test)
        mlkr_acc_list.append(metrics.accuracy_score(PCA_reduced_y_test, Y_pred_mlkr))
        print("n_neighbours||K: ", K, "Accuracy:", mlkr_acc_list[-1])
    mlkr_plotting(mlkr_acc_list)


def mlkr_plotting(acc_list):
    print(acc_list)
