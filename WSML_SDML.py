...
def SDML_experiment(K_list):
    ...
    sdml_acc_list = []
    sdml = SDML(random_state=1234, verbose=True,balance_param=0.5,preprocessor=None)
    sdml.fit(pairs=train_pairs, y=np.squeeze(y_pairs))
    sdml_metric = sdml.get_metric()
    for K in K_list:
        knn_sdml = KNeighborsClassifier(n_neighbors=K, metric=sdml_metric)
        knn_sdml.fit(PCA_reduced_x_train, PCA_reduced_y_train)
        Y_pred_sdml = knn_sdml.predict(PCA_reduced_x_test)
        sdml_acc_list.append(metrics.accuracy_score(PCA_reduced_y_test, Y_pred_sdml))
        print("n_neighbours||K: ", K, "Accuracy:", sdml_acc_list[-1])
    sdml_plotting(sdml_acc_list)


def sdml_plotting(acc_list):
    print(acc_list)
